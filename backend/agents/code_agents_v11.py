...
code_validator_agent = Agent(
    ...
    backstory=(
        ...
        "You do not attempt to fix the code — just report whether it is syntactically valid."
    ),
...
code_refinement_agent = Agent(
    ...
    backstory=(
        "You are a senior algorithmic trading engineer. When given Python code that failed validation, "
        "you analyze the error message, correct the problem, and return fully functional QuantConnect-compatible code."
    ),
    tools=[],
    allow_delegation=False
)
