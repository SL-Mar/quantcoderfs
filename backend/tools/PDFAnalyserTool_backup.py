...
class PDFAnalyserTool(BaseTool):
    ...
    def _run(self, pdf_path: str) -> List[Dict[str, Any]]:
        ...
        logging.info(f"[PDFAnalyserTool] Extracted {len(sections)} sections before truncation.")
        ...
        for sec in sections:
            ...
            token_count = len(enc.encode(sec_text))
        ...
        logging.info(f"[PDFAnalyserTool] Returning {len(trimmed_sections)} sections capped at {total_tokens} tokens.")
